...
**当前动作**：第一刀已合入 `main`（PR #18 / merge `ef97229`）：GHA Postgres/Redis service、PostgreSQL 驱动、fresh 库迁移到唯一 head 与 Redis ping 已在 CI 验证。本机仍无 Docker/WSL/PostgreSQL，因此故障注入、多 worker 复跑和备份恢复尚未执行；在这些证据出现前本卡不得标 `done`。
...
- 实验记录：[20260831-R2-05-postgres-redis-gha.md](./experiments/20260831-R2-05-postgres-redis-gha.md)
