...
100k 不进普通 PR，需手工触发 `.github/workflows/p1-code-scale.yml`。
...
- R2-05 仍不得标 `done`；剩余 100k 实测与跨 worker 登录限流。
