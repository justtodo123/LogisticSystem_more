...
> **修订日期**：2026-08-31。R2-02 已通过 PR #10 / #11 / #12 验证并合并；R2-03 已通过 PR #15 / CI 验证并合并；R2-04B 已通过 PR #16 / CI 验证并合并。R2-05 已启动 GHA Postgres/Redis 第一刀，仍不得标 done。
...
| 05 | P1 | P1 | in_progress | [PostgreSQL、Redis 与故障韧性](./05-postgresql-redis-resilience.md) | GHA Postgres/Redis 基线进行中；未完成故障注入与多 worker 验收 |
...
当前下一动作：P0 已全部完成。开展 **R2-05** 第一刀（GHA Postgres/Redis + PostgreSQL 驱动）。故障注入、多 worker 复跑完成前本卡不得标 `done`，不得用 SQLite 结果替代。
...
- R2-00A 已完成 Alembic 单 head、正式启动 migration gate、运行时 DDL 移除和 SQLite schema parity；仓库 `docker-compose.yml` 仍是 SQLite + 单 worker，PostgreSQL 驱动、Redis 与多 worker 验证属于 R2-05，不计入当前已完成能力。
