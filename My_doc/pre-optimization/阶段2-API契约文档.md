...
> 
> **更新说明（V1.5）**：
> - 订单创建请求体新增可选字段 `storage_center_code`，不传则系统随机分配存储中心
> - 货物 `node_id` 修复为存储中心（L0）而非目的地（L2）
> - 订单创建响应新增 `storage_center_code` 和 `storage_center_name`
> - 导入模板新增 `storage_center_code` 列（可选）
> 
> **更新说明（V1.4）**：
...
| storage_center_code | string | 否 | 存储中心（L0）编号，不传则系统随机分配 |
...
| 40001 | 指定的存储中心不存在 | storage_center_code 无效 |
| 50000 | 系统中无可用存储中心 | 未初始化演示数据 |
...
| storage_center_code | 存储中心编号 | 否（空则随机分配） |
| time_window | 时效要求 | 是 |
| goods_name | 货物名称 | 是 |
| goods_type | 货物类型 | 是 |
| weight | 重量(kg) | 是 |
